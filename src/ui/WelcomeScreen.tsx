...
export function WelcomeScreen({ onCommand, vaultUnlocked }: WelcomeScreenProps) {
  ...
  const commands = [
    ...
    { cmd: "dashboard", desc: "multi-service status overview", color: colors.green },
    { cmd: "health", desc: "health checks with history & sparklines", color: colors.yellow },
  ...
  return (
    <Box flexDirection="column" width={termWidth} height={termHeight - 4} overflow="hidden" justifyContent="center">
      ...
      <Box marginX={margin} marginBottom={1}>
        <InputBar onSubmit={onCommand} placeholder="discover · connections · dashboard · health · help · quit" />
      </Box>
